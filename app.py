...
ACCOUNT_SID = 'XXXXXX'
AUTH_TOKEN = 'XXXXXXX'
TWILIO_PHONE_NUMBER = os.getenv('TWILIO_PHONE_NUMBER', 'whatsapp:XXXXXXX')
...
def chat():
    ...
    # score = score_p(q, vector_store.similarity_search(q)[0].text)
    scorer = BERTScorer(model_type='bert-base-uncased')
    print(vector_store.similarity_search(q)[0].page_content)
    P, R, F1 = scorer.score([q], [vector_store.similarity_search(q)[0].page_content])
    score =  f"BERTScore Precision: {P.mean():.4f}, Recall: {R.mean():.4f}, F1: {F1.mean():.4f}"
    my_j = {'query': q, 'response': response, 'Precision': P.mean(), 'Recall': R.mean(), 'F1': F1.mean()}
    with open('data_score.json', 'a') as f:
        f.write(str(my_j)+'\n')

    # now soter this data in a csv file
    # with open('data.csv', 'a') as f:
    #     f.write(f"{q},{vector_store.similarity_search(q)[0].page_content},{score}\n")
